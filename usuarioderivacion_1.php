...
$elementos = count($var);
	...
	{
	$respu=mysqli_query($conn, "select * from miderivacion where miderivacion_mi_codigo='$codigo'and miderivacion_su_codigo='$var[$i]'");
	...
	{	
    mysqli_query($conn, "insert into miderivacion (miderivacion_mi_codigo,miderivacion_su_codigo,miderivacion_estado)values('$codigo','$var[$i]','1')") or die("El Registro no Existe");
	}
	else
	{
	    mysqli_query($conn, "insert into miderivacion (miderivacion_mi_codigo,miderivacion_su_codigo,miderivacion_estado)values('$codigo','$var[$i]','0')") or die("El Registro no Existe");
	}
	}
   } //end for
...
<?php
 ...
 {  $var=$_POST['cod_usuario'];
	...
	for($i=0; $i < $elementos; $i++)
	{
	mysqli_query($conn, "delete from miderivacion where miderivacion_mi_codigo='$codigo'and miderivacion_su_codigo='$var[$i]'") or die("El Registro no Existe");
	}
	?>
	<script>
			window.self.location="usuarioderivacion_1.php";
  ...
  ?>
	  <script>
		  window.self.location="usuarioderivacion.php";
	  </script>
  <?php
  }
...
<div style="overflow:auto; width:400; height:250px; align-self:left;">
<table boder=1>
<tr>
...
<?php
$result=mysqli_query($conn, "SELECT * FROM cargos,departamento where cargos.cargos_id='$rows[miderivacion_su_codigo]' and cargos.cargos_cod_depto=departamento.departamento_cod_departamento");
